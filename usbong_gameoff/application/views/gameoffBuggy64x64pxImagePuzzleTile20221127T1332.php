...
//added by Mike, 20221121
iPUZZLE_STAGE_0_NUMBERS=0;
iPUZZLE_STAGE_1_ONE_DREAM=1;
iPUZZLE_STAGE_2_CAMBODIA=2;
...
/*	//edited by Mike, 2021127
	let iPuzzleTileWidth=32;
	let iPuzzleTileHeight=32;
*/
	if (iCurrentPuzzleStage==iPUZZLE_STAGE_1_ONE_DREAM) {
		iPuzzleTileWidth=64;
		iPuzzleTileHeight=64;
		iBorderOffset=-16;

		iPuzzleTileTotalWidthMax=iPuzzleTileWidth*iColumnCountMax;
		iPuzzleTileTotalHeightMax=iPuzzleTileHeight*iRowCountMax;

		iOffsetWidth=iStageMaxWidth/2-iPuzzleTileTotalWidthMax/2+iPuzzleTileTotalWidthMax*0.25;
		iOffsetHeight=iStageMaxHeight/2-iPuzzleTileTotalHeightMax/2+iPuzzleTileTotalHeightMax*0.25;
	}
	else {
		iPuzzleTileWidth=32;
		iPuzzleTileHeight=32;
		iBorderOffset=0;
...
/* //edited by Mike, 20221127
	//edited by Mike, 20221127; from 20221113
	//iPuzzleTileWidth=32;
	//const iBorderOffset=0; //2;
	//iPuzzleTileWidth=64;
	//const iBorderOffset=-16; //2;
...
*/
	...
	if (!bIsInitAutoGeneratePuzzleFromEnd) {
		...
		//if NOT the start stage, i.e. numbers sequence
...
//	for (let iTileBgCount=0; iTileBgCount<16; iTileBgCount++) {		
	for (iRowCount=0; iRowCount<iRowCountMax; iRowCount++) {		
		...
		//added by Mike, 20221127
		arrayPuzzleTileCountId[iTileBgCount].style.transform = "scale(1.5)"
			...
			//alert(iRowCount);
				if ((iRowCount-1)>=0) {
					...
					if (iCurrentPuzzleStage==0) { 					
						...
						arrayPuzzleTileCountId[iTargetTileBgCount].className="Image64x64TileTarget";
					}
					
					
					//added by Mike, 20221119
					puzzleTileImageTargetBorder.style.left = iHorizontalOffset+iOffsetWidth+iPuzzleTileWidth*iColumnCount+iBorderOffset*iColumnCount+"px";
					
					puzzleTileImageTargetBorder.style.top = 0+iOffsetHeight+iPuzzleTileHeight*(iRowCount-1)+iBorderOffset*iRowCount+"px";
...
//				if ((iRowCount+1)<=iRowCountMax-1) {
				...
				//arrayPuzzleTileCountId[iTileBgCount].className="Image32x32TileSpace";
					...
					//added by Mike, 20221119
					puzzleTileImageTargetBorder.style.left = iHorizontalOffset+iOffsetWidth+iPuzzleTileWidth*(iColumnCount-1)+iBorderOffset*iColumnCount+"px";

					//puzzleTileImageTargetBorder.style.left = iOffsetWidth+iPuzzleTileWidth*(iColumnCount-1)+"px";
					
					puzzleTileImageTargetBorder.style.top = 0+iOffsetHeight+iPuzzleTileHeight*(iRowCount)+iBorderOffset*iRowCount+"px";
...
//				if ((iColumnCount+1)<=iColumnCountMax-1) {
			...
			if (iHorizontalOffsetOfTargetBorder	!= iHorizontalOffset) {
				...
				puzzleTileImageTargetBorder.style.left = (puzzleTileImageTargetBorder.style.left.replace("px","")-iHorizontalOffsetOfTargetBorder+iHorizontalOffset)+iBorderOffset*iColumnCount+"px";			
			...
			if ((bIsTargetAtSpace) && (bHasPressedStart)) {
				...
				//edited by Mike, 20221127
					...
					if (iCurrentPuzzleStage==0) { 					
						...
						if (arrayPuzzleTileCountId[iTileBgCount].className=='Image64x64TileSpace') {
						arrayPuzzleTileCountId[iTileBgCount].className="Image64x64TileSpaceTarget";
						}
						else {
						arrayPuzzleTileCountId[iTileBgCount].className="Image64x64TileSpace";
						}
					}					
...
//				for (iCount=0; iCount<iDirectionTotalKeyCount; iCount++) {
					...
					if (iCurrentPuzzleStage==0) { 					
						...
						arrayPuzzleTileCountId[arrayPuzzleTilePos[iRowCount-1][iColumnCount]].className="Image64x64Tile";
					}
	...
	//						if ((iRowCount+1)<iRowCountMax-1) {
					...
					if (iCurrentPuzzleStage==0) { 					
						...
						arrayPuzzleTileCountId[arrayPuzzleTilePos[iRowCount][iColumnCount-1]].className="Image64x64Tile";
					}
			...
			if (iTargetTileBgCount==-1) {
				break;
					...
					if (iCurrentPuzzleStage==0) { 					
						...
						arrayPuzzleTileCountId[iTileBgCount].className="Image64x64Tile";
						
						arrayPuzzleTileCountId[iTargetTileBgCount].className="Image64x64TileSpace";
					}
...
for ($iCount=0; $iCount<$iTileBgCountMax; $iCount++) {
	...
	//count1024x1024.png
?>		
	<img id="puzzleTileImageIdBg<?php echo $iCount;?>" class="Image64x64Tile" onerror="" src="<?php echo base_url('assets/images/count.png');?>" alt="" title="">

<?php
}
?>
	<!-- added by Mike, 20221119 -->
	<div id="divPuzzleTileImageTargetBorderId" class="Image64x64TileTargetBorder"></div>

	<!-- added by Mike, 20221121 -->
	<div id="divPuzzleTileImageSpaceBorderId" class="Image64x64TileSpaceBorder"></div>
